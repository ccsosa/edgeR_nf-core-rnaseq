...
indir <- "/scratch/bis_klpoe/chsos/analysis/DEG"
#name of the folder where the DEG results are available

#contrasts provided for enrichment
################################################################################
#RUNS
#put all in a nested list to run
# param_list <- list(folder=c("SRP356530", #first study
#                             "SRP257474", #second study
#                             "SRP098756"), #third study
#                    contrasts = list(c("LEAVES_D-LEAVES_C"), #first study
#                    c("DT6H-CK0H"), #second study
#                    c("ROOTS_D-ROOTS_C", #third study
#                      "LEAVES_D-LEAVES_C",
#                      "CROWN_D-CROWN_C"))
# ) 

param_list <- list(folder=c("SRP072216" #first study
                          ), #third study
                   contrasts = list(c("LEAVES_D-LEAVES_C","LEAVES_TABA-LEAVES_D"))
)

#folder <- "SRP356530";contrasts <- c("LEAVES_D-LEAVES_C") #DONE
#folder <- "SRP257474";contrasts <- c("DT6H-CK0H") #DONE #TWO STARS
# folder <- "SRP098756";contrasts <- c("ROOTS_D-ROOTS_C",
#                                      #"ROOTS_D-CROWN_C", #delete
#                                      "LEAVES_D-LEAVES_C",
#                                      "CROWN_D-CROWN_C")
...
summ_all_stud_list <- list()
  for(a in 1:length(param_list$folder)){
    ...
    #a <- 1
    #running the code
    out_dir <- "/scratch/bis_klpoe/chsos/analysis/GO_DEG"
    ...
    ################################################################################
    #run with plaza file
    # ta_go_file <- "/scratch/bis_klpoe/chsos/data/TA_GO_PLAZA/go.tae.csv.gz" # plaza no filtered
